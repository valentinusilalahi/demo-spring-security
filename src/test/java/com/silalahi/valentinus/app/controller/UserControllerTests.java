...
public class UserControllerTests {
	...
	public void greeting() throws Exception {
		...
		RequestBuilder builder = MockMvcRequestBuilders.get("/user").with(user(loginUser)).accept(MediaType.TEXT_PLAIN);
		...
		MvcResult result = mockMvc.perform(builder).andExpect(status().isOk())
				...
				.andExpect(content().contentType(contentTypeText)).andExpect(content().string("hello test_user"))
				.andExpect(cookie().exists("XSRF-TOKEN")).andExpect(forwardedUrl(null)).andExpect(redirectedUrl(null))
	...
	public void getEcho() throws Exception {
		RequestBuilder builder = MockMvcRequestBuilders.get("/user/echo/{message}", "abc").accept(MediaType.TEXT_PLAIN);

		MvcResult result = mockMvc.perform(builder).andExpect(status().isOk())
				.andExpect(content().contentType(contentTypeText)).andExpect(content().string("ABC")).andDo(print())
				.andReturn();
	...
	public void getEcho_403() throws Exception {
		...
		mockMvc.perform(builder).andExpect(status().is(HttpStatus.FORBIDDEN.value())).andDo(print());
	}
	...
	public void getEcho_401() throws Exception {
		...
		mockMvc.perform(builder).andExpect(status().is(HttpStatus.UNAUTHORIZED.value())).andDo(print());
	}

	@WithMockUser(roles = "USER")
	@Test
	public void postEcho() throws Exception {
		RequestBuilder builder = MockMvcRequestBuilders.post("/user/echo").contentType(MediaType.APPLICATION_JSON_UTF8)
				.content("{\"message\": \"hello world\"}").with(csrf()).accept(MediaType.APPLICATION_JSON_UTF8_VALUE);

		MvcResult result = mockMvc.perform(builder).andExpect(status().isOk())
				.andExpect(content().contentType(contentTypeJson)).andExpect(content().string("{message=hello world}"))
				.andDo(print()).andReturn();
